...
void VectorBatch::v2mtp(const Matrix &m, VectorBatch &y) const {
  ...
  assert( xc==yc ); assert( yr==mc ); assert( mr==xr );
  ...
  if (trace_progress()) {
    ...
    assert( m.notinf() ); assert( m.notnan() ); assert( m.normf()!=0.f );
    assert( y.notinf() ); assert( y.notnan() ); assert( y.normf()!=0.f );
  }
  if (trace_scalars()) {
    cout << "v2mtp (computes dl): "
         << this->normf() << "x" << m.normf() << " => " << y.normf() << "\n";
  }
}
...
void VectorBatch::outer2(const VectorBatch &x, Matrix &m ) const {
  ...
  if (trace_progress()) {
    assert( this->notinf() ); assert( this->notnan() ); assert( this->normf()!=0.f );
    assert( x.notinf() ); assert( x.notnan() ); assert( x.normf()!=0.f );
    assert( m.notinf() ); assert( m.notnan() ); assert( m.normf()!=0.f );
  }
  if (trace_arrays()) {
    cout << "Outer2 left input:\n"; this->show();
    cout << "Outer2 right input:\n"; x.show();
    cout << "Outer2 output:\n"; m.show();
  }
}
